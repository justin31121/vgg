...
char *keywords[KEYWORDS_CAP] = { "if", "while", "for", "const"};
size_t keywords_len[] = { 2, 5, 3, 4};
size_t keywords_count = 4;
...
bool line_eol(char *buffer, size_t len, bool *special, size_t *end) {
...
void render(SDL_Renderer *rend, Font *font, Buffer *buffer, const Cursor *cursor, bool c) {
    ...
    for(size_t i=0;i<text.lines_count;i++) {
        ...
        for(size_t j=0;j<line.intervals_count;j++) {
            ...
            font_render_text_sized(rend, font, vec(inter.start - origin.x, (i+y)*font->height*scale - origin.y),
		   buffer->buffer + buffer->positions[i] + inter.start, inter.end,
		   scale, inter.color);
        }
